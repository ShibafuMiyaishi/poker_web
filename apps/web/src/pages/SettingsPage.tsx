...
export function SettingsPage({ onLogout }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      <SectionLabel jp="設定" en="Settings" />
      ...
      <section className="relative rounded-md border border-brass/25 bg-gradient-to-b from-ink-deep/95 to-ink/95 p-5 shadow-card">
        ...
        <div className="flex items-baseline gap-2 mb-3">
          <span className="font-jp text-sm text-ivory tracking-widest">アカウント</span>
          <span className="font-display italic text-xs text-brass tracking-widest uppercase opacity-70">
            Account
          </span>
        </div>
        {user ? (
          <div className="text-xs space-y-1.5">
            ...
            <div className="flex items-baseline gap-2">
              <span className="font-jp text-ivory-muted w-20 shrink-0">user_id</span>
              <span className="font-mono-tabular text-[10px] text-ivory-dim">{user.id}</span>
            </div>
          </div>
        ) : (
          <p className="text-xs text-ivory-muted font-display italic">未ログイン</p>
        )}
        <div className="mt-4 flex gap-2 flex-wrap">
          <button
            type="button"
            onClick={startGoogleLogin}
            className="text-xs px-4 py-2 rounded-md brass-surface text-ivory font-display tracking-widest hover:brightness-110 transition"
          >
            Google でサインイン
          </button>
          <button
            ...
            className="text-xs px-4 py-2 rounded-md border border-crimson/40 bg-crimson/10 text-crimson-glow font-display tracking-widest hover:bg-crimson/20 transition"
          >
            ログアウト
          </button>
        </div>
      ...
      <section className="relative rounded-md border border-brass/25 bg-gradient-to-b from-ink-deep/95 to-ink/95 p-5 shadow-card">
        <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-brass/40 to-transparent" />
        <div className="flex items-baseline gap-2 mb-3">
          <span className="font-jp text-sm text-ivory tracking-widest">情報</span>
          <span className="font-display italic text-xs text-brass tracking-widest uppercase opacity-70">
            About
          </span>
        </div>
        <ul className="text-xs text-ivory-dim space-y-1">
          <li>
